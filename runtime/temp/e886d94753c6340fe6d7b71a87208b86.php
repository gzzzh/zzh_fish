<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:91:"C:\phpStudy\PHPTutorial\WWW\xianzhitao\public/../application/admin\view\activity\index.html";i:1550638510;s:78:"C:\phpStudy\PHPTutorial\WWW\xianzhitao\application\admin\view\public\left.html";i:1547878793;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
...
<div class="main-wrap">
    <div class="crumb-wrap">
        <div class="crumb-list"><i class="icon-font"></i><a href="<?php echo url('admin/Index/index'); ?>">首页</a><span class="crumb-step">&gt;</span><span class="crumb-name">商家活动审核</span></div>
    </div>
    <div class="search-wrap">
        <div class="search-content">
            <form action="<?php echo url('admin/activity/index'); ?>" id="myform" name="myform" method="post">
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="status">
                                ...
                                <option value="1" <?php if(isset($status)): if($status == 1): ?>selected<?php endif; endif; ?>>未付款</option>
                                <option value="2" <?php if(isset($status)): if($status == 2): ?>selected<?php endif; endif; ?>>待审核</option>
                                <option value="3" <?php if(isset($status)): if($status == 3): ?>selected<?php endif; endif; ?>>进行中</option>
                                <option value="4" <?php if(isset($status)): if($status == 4): ?>selected<?php endif; endif; ?>>已完成</option>
                                <option value="5" <?php if(isset($status)): if($status == 5): ?>selected<?php endif; endif; ?>>驳回</option>
                                <option value="6" <?php if(isset($status)): if($status == 6): ?>selected<?php endif; endif; ?>>关闭</option>
                                <option value="7" <?php if(isset($status)): if($status == 7): ?>selected<?php endif; endif; ?>>待结算</option>
                            </select>
                        </td>

                        <td>
                            <input type="text" name="mobile" value="<?php echo (isset($mobile) && ($mobile !== '')?$mobile:''); ?>" placeholder="请输入手机号搜索">
                        </td>
        ...
        <div class="result-content">
            <table class="result-tab" width="100%">
                ...
                <tr>
                    ...
                    <td><img src="<?php echo $vo['product_logo']; ?>"></td>
                    <td><?php echo $vo['product_link']; ?></td>
                    <td><?php echo $vo['product_price']; ?> | <?php echo $vo['num']; ?></td>
                    <td><?php echo $vo['begin_time']; ?>——<?php echo $vo['end_time']; ?></td>
                    <td>
                        <?php if($vo['status'] == 1): ?><span style="color: pink">未付款</span>
                        <?php elseif($vo['status'] == 2): ?> <span style="color: red">待审核</span>
                        <?php elseif($vo['status'] == 3): ?> <span style="color: darkorange">进行中</span>
                        <?php elseif($vo['status'] == 4): ?> <span style="color: sandybrown">已完成</span>
                        <?php elseif($vo['status'] == 5): ?><span style="color: red">驳回</span>
                        <?php elseif($vo['status'] == 6): ?><span style="color: red">关闭</span>
                        <?php elseif($vo['status'] == 7): ?><span style="color: red">待结算</span>
                        <?php endif; ?>
                    </td>

                    <td>
                        总费用: <?php echo $vo['deposit'] + $vo['service'] + $vo['adv_price']; ?><br>
                        保证金：<?php echo $vo['deposit']; ?><br>
                        推广费用： <?php echo $vo['service']; ?><br>
                        增值费用： <?php echo $vo['adv_price']; ?>
                    </td>


                    <td>
                        <a href="<?php echo url('admin/activity/details',['id' => $vo['id']]); ?>" target="_blank">活动详情</a>
                        <?php if($vo['status'] == 2): ?>
                            <a href="<?php echo url('admin/activity/actAudit',['id' => $vo['id'],'userid' => $vo['userid'],'type' => 'pass']); ?>">通过</a>
                            <a href="<?php echo url('admin/activity/actAudit',['id' => $vo['id'],'userid' => $vo['userid'],'type' => 'reject']); ?>">驳回</a>
                        <?php elseif($vo['status'] == 7): ?>
                            <a href="<?php echo url('admin/activity/closeAnAccountUser',['id' => $vo['id'],'userid' => $vo['userid'],'type' => 'close']); ?>" target="_blank">结算</a>
                        <a href="<?php echo url('admin/activity/closeAnAccountUser',['id' => $vo['id'],'userid' => $vo['userid'],'type' => 'reject']); ?>" target="_blank">驳回</a>
                        <?php endif; ?>
                    </td>
                </tr>
